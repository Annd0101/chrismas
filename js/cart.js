...
const addCartToHTML = () => {
  ...
  if (cart.length > 0) {
    cart.forEach((item) => {
      ...
      newItem.innerHTML = `
            ...
            `;
      // summary product
      itemSummary.innerHTML = `<div class="w-100 d-flex justify-content-between mt-3"> <h6 class="text-success fw-bold">${
        info.name
      }</h6>
            <h6 class="fw-bold">$${info.price * item.quantity}</h6> </div>`;
    });
    // summary price
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    cupponElement.addEventListener("input", () => {
      var cupponElement = document.querySelector(".cuppon");
