...
const addCartToHTML = () => {
  ...
  if (cart.length > 0) {
    ...
    priceSummary.innerHTML = `   <h5 class="text-uppercase fw-bold">Total price</h5>
                  <h5 class="text-success fw-bold total">$${total} </h5>`;
    // apply shipping 
    document.addEventListener('DOMContentLoaded', function() {
      var selectElement = document.querySelector('.select');
      var totalElement = document.querySelector('.total');
      var cupponElement = document.querySelector('.cuppon')
      selectElement.addEventListener('change', calculateTotal);
      cupponElement.addEventListener('input', calculateTotal)
      calculateTotal()
  });
    function calculateTotal() {
      ...
      var cupponElement = document.querySelector('.cuppon')
          ...
          let initialPrice = total
          let totalAfterShip = 0;
          switch (selectedValue) {
            case 1: 
              totalAfterShip = initialPrice * 1.1;
              break;
            case 2:
              totalAfterShip = initialPrice * 1.3;
              break;
            case 3: 
              totalAfterShip = initialPrice * 1.5;
              break;
            case 4: 
              totalAfterShip = initialPrice * 2;
              break;
          }
        // Apply coupon code discount if valid
          let cupponCode = cupponElement.value
          switch (cupponCode) {
            case '1234': 
              totalAfterShip = totalAfterShip * 0.5;
            default:
              totalAfterShip = totalAfterShip * 1;
              alert("Apply shipping and cuppon successfully")
          }
          // Hiển thị total
          totalElement.textContent = totalAfterShip.toFixed(2);
      }
  }
};
addCartToHTML();
